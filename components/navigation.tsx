      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3 md:py-5 lg:px-8">
          ...
          <nav className="hidden items-center gap-8 md:flex lg:gap-10">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-sm font-medium uppercase tracking-[0.2em] text-gray-light transition-colors duration-300 hover:text-cream"
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="flex items-center gap-4 md:hidden">
            ...
            >
              <span className={`block h-px w-6 origin-center bg-cream transition-all duration-300 ${isOpen ? "translate-y-[6px] rotate-45" : ""}`} />
              <span className={`block h-px w-6 bg-cream transition-all duration-300 ${isOpen ? "opacity-0" : ""}`} />
              <span className={`block h-px w-6 origin-center bg-cream transition-all duration-300 ${isOpen ? "-translate-y-[6px] -rotate-45" : ""}`} />
            </button>
          </div>
        </div>
